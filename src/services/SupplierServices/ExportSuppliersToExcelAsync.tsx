...
export const ExportSuppliersToExcelAsync = async () => {
  try {
    const jwtToken = getCookie("jwtToken");

    const response = await fetch(
      "http://localhost:5004/api/Supplier/exportSuppliersToExcel",
      {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
      ...
      console.error("Error exporting suppliers:", response.statusText);
      return;
    }
    ...
    a.href = url;
    a.download = "suppliers.xlsx"; // Имя файла, которое будет предложено для загрузки
